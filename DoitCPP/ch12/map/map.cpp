...
int main() {
    ...
    // 맵 크기 출력
    std::cout << "map 크기: " << scores.size() << std::endl;

    // 특정 키에 해당하는 값 검색
    auto it = scores.find("Bob");
    if (it != scores.end()) {
        std::cout << "Bob의 점수 검색 결과: " << it->second << std::endl;
    }
    else {
    ...
    // 맵 크기 출력
    std::cout << "Bob 정보 제거 후, map 크기: " << scores.size() << std::endl << std::endl;

    // 맵을 순회하며 모든 키와 값 출력
    std::cout << "---map 모든 원소 출력---" << std::endl;
