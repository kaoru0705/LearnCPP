...
public:
    ...
    monster_b(const monster_b& ref);    // 복사 생성자 선언(오버로딩) 여기서 몸체를 정의해도 된다.
...
monster_b::monster_b(const monster_b& ref)      // 복사 생성자 정의(깊은 복사) 정의를 클래스 밖에서 적으려면 항상 monster_b::를 붙여야 된다.
