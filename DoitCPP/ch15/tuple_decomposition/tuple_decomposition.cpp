...
int main() {
    ...
    // 튜플의 개별 변수에 매칭
    std::tie(monster_type, std::ignore, hp, power) = get_monster_status(monster_a_inst);

    print_out_tuple(monster_a_status);
    std::cout << "tie()로 변수에 매핑" << std::endl << "monster type(" << monster_type << ") : hp(" << hp << "), power(" << power << ")" << std::endl;
